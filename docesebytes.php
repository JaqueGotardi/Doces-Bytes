...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['action'])) {
		...
		if ($action === 'add_comanda') {
			...
			if (empty($mesa) || empty($cliente_nome) || !is_array($itens_array) || count($itens_array) == 0) {
				...
				$numero_diario = $pdo->lastInsertId();
				$_SESSION['msg'] = "Comanda cadastrada com sucesso. Número do dia: $numero_diario";
				notifyKitchen($numero_diario, "Nova comanda criada e enviada para preparo.");
				logAction($pdo, $usuario, "criar_comanda", "ID: $numero_diario");
			}
	...
	if (!is_array($novos_itens_array) || count($novos_itens_array) == 0) {
		...
		if (!$comanda) {
			...
			$stmtUpdate->execute([json_encode($itens_atual), $numero_diario]);
			$_SESSION['msg'] = "Itens adicionados com sucesso.";
			logAction($pdo, $usuario, "adicionar_itens_comanda", "Comanda ID: $numero_diario");
		}
	}
    ...
    if ($nivel_acesso != 1) {
        ...
        $statuscli = isset($_POST['statuscli']) ? $_POST['statuscli'] : '';
        ...
        if (empty($nome) || empty($cpf) || empty($data_nascimento) || empty($endereco) ||
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO cliente (nome, cpf, data_nascimento, endereco, telefone, whatsapp, email, statuscli) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$nome, $cpf, $data_nascimento, $endereco, $telefoneCompleto, $whatsappCompleto, $email, $statuscli]);
                ...
                if ($e->getCode() == 23000 && str_contains($e->getMessage(), 'cpf')) {
                    $_SESSION['msg'] = "Erro: CPF já existente.";
                } else {
    ...
    $id_comanda = isset($_POST['numero_diario']) ? intval($_POST['numero_diario']) : 0;
    $numero_diario = $id_comanda;
    ...
    $stmt->execute([$novo_status, $pagamento_json, $id_comanda]);
    ...
    <p><strong>Código da Comanda:</strong> " . htmlspecialchars($numero_diario) . "</p>
    ...
    <p><strong>Último Pagamento:</strong><br>
    <strong>Métodos de Pagamento:</strong> " . implode(", ", $metodos) . "<br>
    <strong>Valor:</strong> R$ " . number_format($valor_recebido, 2, ',', '.') . "<br>
    <strong>Data/Hora do Pagamento:</strong> $data_hora<br>
    <strong>Caixa Responsável:</strong> " . htmlspecialchars($usuario) . "</p>";
    ...
    foreach ($pagamento_atual['historico'] as $pagto) {
        echo "<p style='margin: 5px 0;'>
            <strong>Método:</strong> " . implode(", ", $pagto['metodos']) . "<br>
    ...
    // Impede que a página continue renderizando o restante (como home)
    exit;
}
...
case 'cadastrar':
    ...
    if (isset($_SESSION['msg'])) {
        ...
        echo "<br><form method='post' action='docesebytes.php' onsubmit='return validateItems()' style='max-width: auto; margin: 0 auto;'>
        ...
        echo "  </select><br>
                ...
                <div id='item_entry' style='display:none; margin-top:5px;'>
                    ...
                    <br><button type='button' onclick='itemManager_comanda.adicionarItem()' class='buttonteste'>➕ Adicionar Item</button>
                </div><br><br>
...
case 'adicionar':
	if ($_SERVER['REQUEST_METHOD'] === 'POST' 
		...
		$numero_diario = intval($_POST['id_comanda']);
		$novos_itens_json = trim($_POST['novos_itens']);
		$novos_itens_array = json_decode($novos_itens_json, true);
        ...
        header("Location: docesebytes.php?page=comandas&subpage=adicionar&id=$numero_diario");
        exit;
    }
...
";
        ...
        echo "<form method='get' action='docesebytes.php'>
				...
				<label>Cliente:</label>
				<input type='text' name='filtro_cliente' placeholder='Digite o nome do cliente'><br>
...
case 'cancelamentos':
	...
	break;
			...
			case 'relatorio':
				...
				$dadosFiltrados = isset($_GET['status']) || isset($_GET['numero_diario']);
				...
				if ($dadosFiltrados) {
					...
					if ($comandas) {
                        ...
                        echo "<br><button onclick='window.print()' class='buttonteste'>🖨️ Imprimir Relatório</button>";
                    } else {
            ...
            case 'relatorio':
				...
				if ($categoria || $status) {
                    ...
                    if ($produtos) {
                        ...
                        echo "<br><br></tbody></table><br><button onclick='window.print()' class='buttonteste'>🖨️ Imprimir Relatório</button>";
                    } else {
...
echo "<h3>Cadastrar Cliente</h3>
      <form method='post' action=''>
        ...
        <label>Status:</label>
        <select name='statuscli' required>
          <option value='ativo'>Ativo</option>
          <option value='inativo'>Inativo</option>
        </select><br>
...
case 'pagamento_encerrar':
    ...
    echo "<form method='post' action='docesebytes.php'>
        ...
        <input type='hidden' name='id_comanda' value='" . $id . "'> <!-- Corrigido aqui -->
